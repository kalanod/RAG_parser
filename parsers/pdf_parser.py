    ...
    def _apply_size_constraints(
        ...
        *,
        ...
        for block in sized:
            ...
            if block.type == buffer.type:
                merged_text = self._merge_block_text(buffer, block)
                if len(merged_text) <= max_block_chars:
                    buffer = _Block(text=merged_text, type=buffer.type)
                    continue
        ...
        for block in combined:
            if (
                block.type != "heading"
                and final_blocks
                and final_blocks[-1].type == block.type
            ):
                merged_text = self._merge_block_text(final_blocks[-1], block)
                if (
                    len(merged_text) <= max_block_chars
                    and (
                        len(final_blocks[-1].text) < min_block_chars
                        or len(block.text) < min_block_chars
                    )
                ):
                    final_blocks[-1] = _Block(text=merged_text, type=block.type)
                    continue
